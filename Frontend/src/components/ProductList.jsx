...
const ProductList = () => {
  ...
  const { token, logout } = useAuth(); // Assuming useAuth provides token and logout
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/vendors/delete-product/${productToDelete}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setProducts(products.filter((product) => product._id !== productToDelete));
      setShowModal(false);
      setProductToDelete(null);
      alert("Product deleted successfully"); // Replace with toast in production
    } catch (error) {
      const message = error.response?.data?.message || error.message;
